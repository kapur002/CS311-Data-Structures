...
class stack
{
 ...
 public: // prototypes to be used by the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: variable to receieve the top element which is passed by reference.
  void topElem(el_t&);

  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: function of boolean type to determine if stack is empty.
  bool isEmpty();
  //PURPOSE: function of boolean type to determine if stack is full.
  bool isFull();
  //PURPOSE: displays all the items in the stack
  void displayAll();
  //PURPOSE: clears the stack, pops all the elements, with local variable "elem".
  void clearIt();
